<form action="{{ route('posts') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="mt-2 w-full flex justify-start text-gray-500">
        ...
        <div class="w-4/12 lg:w-2/12 lg:ml-2">
            <input class="mt-2 py-2 px-2 rounded-md shadow border-gray-300  w-full" type="text" name="interchange" placeholder="インターチェンジ名" value="{{ old('interchange') }}" required="required">
        </div>
        <div class="w-4/12 lg:w-2/12 lg:ml-2">
            <input class="mt-2 py-2 px-2 rounded-md shadow border-gray-300  w-full" type="text" name="interchange" placeholder="場所名を入れてー" value="{{ old('interchange') }}" required="required">
        </div>
    </div>
</form>
