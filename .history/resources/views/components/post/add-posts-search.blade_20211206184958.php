<form action="{{ route('posts') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="text-sm　mt-2 w-full flex flex-wrap flex-row justify-between lg:justify-start text-gray-500">
        <div class="w-6/12 lg:w-2/12">
            <x-post.add-post-form-genre name="term" data="" value="{{ config('postTerms.parentTerms')[old('term')] }}"></x-post.add-post-form-genre>
        </div>
        <div class="w-6/12 lg:w-2/12">
            <x-post.add-post-form-genre name="subterm" data="" value="{{ config('postTerms.parentTerms')[old('subterm')] }}"></x-post.add-post-form-genre>
        </div>
        <div class="w-6/12 lg:w-2/12">
