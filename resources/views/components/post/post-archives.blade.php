<article class="poRe postsArchives">
    <div class="ScrollWap">
        <form action="{{ route('posts') }}" method="post">
            @csrf
            <div class="postsArchivesWap">
                @foreach($datas as $val)
                <div class="postsArchivesLoop">
                    ...
                    <div class="display_flex_center dateGenrePosts">
                        <p class="datePosts">
                            日にち：{{ $val['travelDate'] }}【 {{ $val['arrivalTime'] }}】({{ number_format($val['cost']) }}円)
                        </p>
                        <p class="GenrePosts">
                            {{ config('postTerms.parentTerms')[$val['term']] }}&nbsp;/&nbsp;{{ config('postTerms.parentTerms')[$val['subTerm']] }}
                        </p>
                    </div>
                    <p class="txtPostsInfo">【{{ $val['prefectures'] }}】【{{ $val['interchange'] }}】【{{ $val['stayTime'] }}時間】</p>
                    <a class="underline" href="{{ $val['url'] }}" target="_blank">{{ Str::limit($val['placeName'], 107, '...') }}</a>
